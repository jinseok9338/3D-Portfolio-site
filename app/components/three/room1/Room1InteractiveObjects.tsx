...
import { ROOM1_INTERACTIVE_MESHES, ROOM1_OBJECT_MESHES, getMeshObjectId } from './config';
...
// Hover 시 표시할 간단한 레이블
const HOVER_LABELS: Record<Room1ObjectId, string> = {
  computer: '소개',
  desk: '경력',
  shelf: '기술 스택',
  telescope: '목표',
...
export function Room1InteractiveObjects() {
  ...
  // Room1이 아니면 아무것도 렌더링 안 함
  if (activeRoom !== 'Room1') {
    return null;
  }

  const isRoom1Hovered = hoveredObject && ['computer', 'desk', 'shelf', 'telescope'].includes(hoveredObject);

  return (
    <>
      {/* Hover 시 간단한 레이블 표시 (클릭된 오브젝트가 없을 때만) */}
      {isRoom1Hovered && hoverPosition && !activeObject && (
        <Html
        ...
        >
          <div className="px-2 py-1 bg-background/90 backdrop-blur-sm border border-primary/50 rounded text-xs text-foreground whitespace-nowrap">
            {HOVER_LABELS[hoveredObject as Room1ObjectId]}
          </div>
        </Html>
      )}
